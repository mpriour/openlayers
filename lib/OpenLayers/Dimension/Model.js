...
OpenLayers.Dimension.Model = OpenLayers.Class({
     ...
     */
    EVENT_TYPES : ["rangemodified", "listmodified", "addlayer", "removelayer"],  
    ...
    removeDimensionInfo: function(info) {
        var rangeMod = valMod = resMod = false;
        var range = this.range || [Number.MAX_VALUE,-1*Number.MAX_VALUE];
        if(info.values){
        ...
        if(info.resolution){
            if(info.resolution == this.resolution){
                var oldRes = this.resolution;
                this.resolution = this.getMinimumResolution();
                if(oldRes != this.resolution){
                    resMod = true;
                }
            }
        }
