...
OpenLayers.Control.DimensionManager = OpenLayers.Class(OpenLayers.Control, {
    ...
    incrementValue : function(step) {
        step = step || this.step;
        var newVal;
        if(this.dimension == 'time' && this.timeUnit){
            newVal = this.incrementTimeValue(step);
        } else {
    ...
    incrementTimeValue : function(step, timeUnits, value){
        ...
        var newPart = parseFloat(value['getUTC'+timeUnit]()) + parseFloat(step);
        value['setUTC'+stepUnit](newPart);
    ...
    guessPlaybackRate : function(model) {
        if(!this.agents) {
            ...
            if(model.values) {
                ...
                return model.guessPlaybackRate();
            }
        }
    },
